...
private:
    ...
    //  Buyer buyer;
public:
    Storre(std::string imy, std::string whereitis, List goods) {
        name = imy;
        address = whereitis;
        goodz = goods;
    }
    ...
    Storre operator+(Storre& other) {
        ...
        for (i = 0; i < other.goodz.Size(); i++) {
            ...
            if (!exists) this->goodz.AddHead(other.goodz[i]);
        }
        other.goodz.Clean();
        return *this;
    }
                    ...
                    if (wishlist[i]->GetCount() > 0) {
                        ...
                        if (money >= payment) {
                            ...
                            else
                            {
                                ...
                                std::cout << "Товар " << wishlist[i]->GetName() << " был куплен в кол-ве " << wishlist[i]->GetCount() << " шт. за " << payment << " руб., в магазине " << Store.name << " по адресу " << Store.address << std::endl;
                            }
                        }
                        else std::cout << "Недостаточно денег для покупки товара " << wishlist[i]->GetName() << " в кол-ве " << wishlist[i]->GetCount() << " шт. в магазине " << Store.name << " по адресу " << Store.address << ": у покупателя " << money << " руб., а пак стоит " << payment << " руб." << std::endl;
                    }
                }
            }
...
public:
    ...
    /*List(Good goodz, Arrow* dalee, int size) {
        head->data = goodz;
        head->next = dalee;
        max_size = size;
    };
